...
class cycleGAN(object):
    ...
    def __init__(
        ...
        g_cycle_cost = tf.reduce_mean(tf.squared_difference(self.input_A,cycl_A)) + tf.reduce_mean(tf.squared_difference(self.input_B,cycl_B))


        self.g_cost_A = g_cost_A + g_cycle_cost
        self.g_cost_B = g_cost_B + g_cycle_cost
        ...
        for epoch in range(self.epochs):
            ...
            for X_batch_A, X_batch_B in zip(batches_A,batches_B):
                ...
                if total_iters % self.save_sample ==0:
                    ...
                    plt.subplot(1,2,1)
                    plt.imshow(255-X_batch_A.reshape(n_H,n_W,n_C))
                    plt.axis('off')
                    plt.subplot(1,2,2)
                    plt.imshow(255-sample.reshape(n_H,n_W,n_C))
                    plt.axis('off')
