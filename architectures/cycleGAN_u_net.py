...
class cycleGAN_u_net(object):
    ...
    def __init__(
        ...
        D_A = pix2pixDiscriminator(self.input_A, d_sizes_A, 'A')
        D_B = pix2pixDiscriminator(self.input_B, d_sizes_B, 'B')

        G_A_to_B = pix2pixGenerator(self.input_A, self.n_H_B, self.n_W_B, g_sizes_enc_A, g_sizes_dec_B, 'A_to_B')
        G_B_to_A = pix2pixGenerator(self.input_B, self.n_H_A, self.n_W_A, g_sizes_enc_B, g_sizes_dec_A, 'B_to_A')
        ...
        with tf.variable_scope('discriminator_B') as scope:     
            predicted_real_B = D_B.d_forward(self.input_A, self.input_B)
        
        with tf.variable_scope('discriminator_B') as scope:
            scope.reuse_variables()
            predicted_fake_B = D_B.d_forward(self.input_A, sample_images_B, reuse=True)

        with tf.variable_scope('discriminator_A') as scope:
            
            predicted_real_A = D_B.d_forward(self.input_B, self.input_A)

        with tf.variable_scope('discriminator_A') as scope:
            scope.reuse_variables()
            predicted_fake_A = D_B.d_forward(self.input_B, sample_images_A, reuse=True)
        ...
        if cost_type == 'GAN':
            ...
            #     logits=predicted_real_A,
            #     labels=tf.ones_like(predicted_real_A)
            # )
            # # #cost is low if fake generated images are predicted as fake (0)
            # d_cost_fake_A = tf.nn.sigmoid_cross_entropy_with_logits(
            #     logits=predicted_fake_A,
            #     labels=tf.zeros_like(predicted_fake_A)
            # )

            
            # self.d_cost_A = tf.reduce_mean(d_cost_real_A) + tf.reduce_mean(d_cost_fake_A)
            
            predicted_real_A= tf.nn.sigmoid(predicted_real_A)
            predicted_fake_A=tf.nn.sigmoid(predicted_fake_A)

            d_cost_real_A = tf.log(predicted_real_A + EPS)

            #d_cost_fake is low if fake images are predicted as real
            d_cost_fake_A = tf.log(1 - predicted_fake_A +EPS)

            self.d_cost_A =  tf.reduce_mean(-(d_cost_real_A + d_cost_fake_A))
            ...
            #     logits=predicted_fake_B,
            #     labels=tf.zeros_like(predicted_fake_B)
            # )

            # self.d_cost_B = tf.reduce_mean(d_cost_real_B) + tf.reduce_mean(d_cost_fake_B)
            
            predicted_real_B= tf.nn.sigmoid(predicted_real_B)
            predicted_fake_B=tf.nn.sigmoid(predicted_fake_B)

            d_cost_real_B = tf.log(predicted_real_B + EPS)

            #d_cost_fake is low if fake images are predicted as real
            d_cost_fake_B = tf.log(1 - predicted_fake_B +EPS)

            self.d_cost_B =  tf.reduce_mean(-(d_cost_real_B + d_cost_fake_B))
            ...
            #         logits=predicted_fake_A,
            #         labels=tf.ones_like(predicted_fake_A)
            #     )
            # )
            self.g_cost_GAN_A = tf.reduce_mean(-tf.log(predicted_fake_A + EPS))
            ...
            #     tf.nn.sigmoid_cross_entropy_with_logits(
            #         logits=predicted_fake_B,
            #         labels=tf.ones_like(predicted_fake_B)
            #     )
            # )

            self.g_cost_GAN_B = tf.reduce_mean(-tf.log(predicted_fake_B + EPS))

            #cycle cost is low if cyclic images are similar to input images (in both sets)
            self.g_cost_cycle_A = tf.reduce_mean(tf.square(self.input_A-cycl_A)) 
            self.g_cost_cycle_B = tf.reduce_mean(tf.square(self.input_B-cycl_B))

            #try this
            # g_cycle_cost_A = tf.reduce_mean(tf.abs(self.input_A- sample_images_A)) 
            # g_cycle_cost_B = tf.reduce_mean(tf.abs(self.input_B- sample_images_B))
