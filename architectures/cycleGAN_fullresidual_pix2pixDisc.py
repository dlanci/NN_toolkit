...
class cycleGAN_fullresidual_pix2pixDisc(object):
    ...
    def __init__(
        ...
        with tf.variable_scope('discriminator_A') as scope:
            ...
            logits_A = D_A.d_forward(self.input_B, self.input_A)
        ...
        with tf.variable_scope('discriminator_B') as scope:
            ...
            logits_B = D_B.d_forward(self.input_A, self.input_B)
        ...
        with tf.variable_scope('discriminator_A') as scope:
            ...
            sample_logits_A = D_A.d_forward(self.input_B, sample_images_A, reuse=True)

        with tf.variable_scope('discriminator_B') as scope:
            scope.reuse_variables()
            sample_logits_B = D_B.d_forward(self.input_A, sample_images_B, reuse=True)
            ...
            for X_batch_A, X_batch_B in zip(batches_A,batches_B):
                ...
                for i in range(discr_steps):
                    ...
                    if self.cost_type=='WGAN-gp':
                        _, d_cost_B, d_gp_B = self.session.run(
                        (self.d_train_op_B, self.d_cost_B, self.gradient_penalty_B),
                        feed_dict={self.input_A:X_batch_A, self.input_B:X_batch_B, self.batch_sz:bs},
                        )
                ...
                for i in range(discr_steps):
                    ...
                    if self.cost_type=='WGAN-gp':
                        ...
                        (self.d_train_op_A, self.d_cost_A, self.gradient_penalty_A),
                        feed_dict={self.input_A:X_batch_A, self.input_B:X_batch_B, self.batch_sz:bs},
                        )
